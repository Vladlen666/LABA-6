...
TEST(ChainHashTable, can_get_size_of_table)
{
  Tab t;
...
TEST(ChainHashTable, insert_elem_increases_size_of_table)
{
  Tab t;
...
TEST(ChainHashTable, cant_change_existing_elem_in_table)
{
  Tab t;
  t.Insert(PolinomObj::Create("name", "xyz"));
...
TEST(ChainHashTable, delete_elem_decreases_size_of_table)
{
  Tab t;
  t.Insert(PolinomObj::Create("name", "xyz"));
  t.Delete("name");
...
TEST(ChainHashTable, can_delete_non_existing_elem_from_table)
{
  Tab t;
  ASSERT_NO_THROW(t.Delete("x"));
}

TEST(ChainHashTable, delete_non_existing_elem_doesnt_change_size_of_table)
{
  Tab t;
  t.Insert(PolinomObj::Create("name", "xyz"));
  t.Delete("bob");
